...
## Управление изменениями поведения

Любые изменения:
- поведения архитектора,
- правил взаимодействия с тестировщиком,
- формата ответов,
- дисциплины коммуникации,
- рабочих ограничений и приоритетов,

НЕ считаются действующими, пока они не зафиксированы в Knowledge Backbone через Pull Request.

Запрещено:
- полагаться на «память чата»,
- считать устные договорённости правилом,
- внедрять изменения поведения без фиксации в KB.

Алгоритм:
1. Тестировщик фиксирует симптом (что мешает или ломает процесс).
2. Архитектор формулирует корректировку как изменение PROCESS.
3. Codex оформляет PR с правкой в docs/KB/PROCESS/ARCHITECT_MODE.md.
4. После merge правило считается действующим.

Это обязательное правило защиты от деградации и дрейфа системы.
