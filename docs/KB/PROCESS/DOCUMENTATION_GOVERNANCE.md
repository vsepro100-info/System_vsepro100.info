...
## Explicit PO verdict command (mandatory)
Владелец продукта обязан выдавать вердикт явно и отдельно.
Формат вердикта: MERGE APPROVED или STOP.
Без явного вердикта дальнейшие действия запрещены.

## Exact instruction specification for Codex (mandatory)
- Каждая задача, выдаваемая Codex, ДОЛЖНА явно указывать:
  - точный документ/файл для редактирования или создания
  - точный текст, который нужно записать
  - точный язык содержимого
- Инструкции ДОЛЖНЫ быть записаны слово-в-слово как конечное содержание.
  Codex не имеет права переосмыслять, переводить, перефразировать или оптимизировать формулировки.
- Исполнение Codex строго механическое:
  копировать, вставить, сохранить.
- Если владелец продукта явно не указывает
  документ, содержание и язык,
  Codex ДОЛЖЕН ОСТАНОВИТЬСЯ и запросить уточнение.
- Любое отклонение от заданного текста, языка
  или целевого документа является нарушением процесса.
